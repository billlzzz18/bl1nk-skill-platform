...
const ALLOWED_EXTENSIONS = [
  ...
  ".swift",
  // Edge cases
  // https://github.com/bl1nk-org/bl1nk/issues/880
  ".py",
  // https://github.com/bl1nk-org/bl1nk/issues/1221
  ".php",
];
...
//
// ex: https://github.com/bl1nk-org/bl1nk/issues/727
const EXCLUDED_DIRS = [
  "node_modules",
  ".git",
  "dist",
...
async function formatFile({
  ...
  try {
    ...
    if (!shouldReadFileContents({ filePath, normalizedRelativePath })) {
      return `<bl1nk-file path="${normalizedRelativePath}">
...
export async function extractCodebase({
  ...
  const isSmartContextEnabled =
    (settings?.enableBl1nkPro || settings?.enableDyadPro) && settings?.enableProSmartFilesContextMode;
