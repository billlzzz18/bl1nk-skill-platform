...
interface ErrorBannerProps {
  error: { message: string; source: "preview-app" | "bl1nk-app" } | undefined;
  onDismiss: () => void;
  onAIFix: () => void;
}
...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Add a little chip that says "Internal error" if source is "bl1nk-app" */}
      {error.source === "bl1nk-app" && (
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : error.source === "bl1nk-app"
                ? "Try restarting the bl1nk app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    if (!selectedComponentsPreview || selectedComponentsPreview.length === 0) {
      if (iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          { type: "deactivate-bl1nk-component-selector" },
          "*",
        );
      }
      setIsPicking(false);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "bl1nk-component-selector-initialized") {
        setIsComponentSelectorInitialized(true);
        return;
      }

      if (event.data?.type === "bl1nk-component-selected") {
        console.log("Component picked:", event.data);

        // Parse the single selected component
        const component = event.data.component
          ? parseComponentSelection({
              type: "bl1nk-component-selected",
      ...
      if (event.data?.type === "bl1nk-component-deselected") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-bl1nk-component-selector"
            : "deactivate-bl1nk-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (
    !data ||
    data.type !== "bl1nk-component-selected" ||
