...
import { hasBl1nkProKey, type UserSettings } from "@/lib/schemas";
...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableBl1nkPro: !settings?.enableBl1nkPro,
    });
  };

  const hasProKey = settings ? hasBl1nkProKey(settings) : false;
  const proModeTogglable = hasProKey && Boolean(settings?.enableBl1nkPro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure bl1nk Pro settings</TooltipContent>
      </Tooltip>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <h4 className="font-medium flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-primary font-medium">bl1nk Pro</span>
            </h4>
          ...
          {!hasProKey && (
            <div className="text-sm text-center text-muted-foreground">
              <Tooltip>
                ...
                <TooltipContent>
                  Visit bl1nk.org/pro to unlock Pro features
                </TooltipContent>
              </Tooltip>
            </div>
          )}
          <div className="flex flex-col gap-5">
            <SelectorRow
              id="pro-enabled"
              label="Enable bl1nk Pro"
              tooltip="Uses bl1nk Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableBl1nkPro)}
              toggle={toggleProEnabled}
            />
            <SelectorRow
              id="web-search"
              label="Web Access"
              tooltip="Allows bl1nk to access the web (e.g. search for information)"
