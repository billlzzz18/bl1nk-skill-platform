...
import { Bl1nkProSuccessDialog } from "@/components/Bl1nkProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showBl1nkProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "bl1nk-pro-return") {
        await refreshSettings();
        showBl1nkProSuccessDialog();
        clearLastDeepLink();
      }
    };
    handleDeepLink();
  ...
  const isBl1nkPro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isBl1nkProEnabled = Boolean(settings?.enableBl1nkPro);
  ...
  return (
    <>
      <div className="@container z-11 w-full h-11 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        {isBl1nkPro && <Bl1nkProButton isBl1nkProEnabled={isBl1nkProEnabled} />}
...
export function Bl1nkProButton({
  isBl1nkProEnabled,
}: {
  ...
  return (
    <Button
      data-testid="title-bar-bl1nk-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isBl1nkProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isBl1nkProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isBl1nkProEnabled && (
