...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as Bl1nkMarkdownParser but convert to clean text
      const formattedContent = convertBl1nkContentToMarkdown(messageContent);
  ...
  // Convert bl1nk content to clean markdown using the same parsing logic as Bl1nkMarkdownParser
  const convertBl1nkContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from Bl1nkMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from Bl1nkMarkdownParser)
  ...
  // Reuse the same parsing functions from Bl1nkMarkdownParser but simplified
