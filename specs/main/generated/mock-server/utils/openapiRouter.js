...
function openApiRouter() {
  return async (request, response, next) => {
    try {
      ...
      // });
      const tags = Array.isArray(request.openapi.schema.tags)
        ? request.openapi.schema.tags
        : [];
      const fallbackController = tags.length ? `${tags[0]}Controller` : undefined;
      const fallbackService = tags.length ? `${tags[0]}Service` : undefined;
      ...
      const serviceName = request.openapi.schema['x-openapi-router-service']
        || fallbackService;
      const controllerModule = controllers[controllerName];
      if (!controllerModule || controllerModule === undefined) {
        handleError(`request sent to controller '${controllerName}' which has not been defined`,
          request, response, next);
      } else {
        const controllerService = Services[serviceName];
        const apiController = typeof controllerModule === 'function'
          ? new controllerModule(controllerService)
          : controllerModule;
        const controllerOperation = request.openapi.schema.operationId;
        const handler = apiController[controllerOperation];
        if (typeof handler !== 'function') {
          handleError(`operation '${controllerOperation}' not found on controller '${controllerName}'`,
            request, response, next);
          return;
        }
